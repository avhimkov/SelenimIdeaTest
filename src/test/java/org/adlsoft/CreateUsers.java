...
public class CreateUsers {
    ...
    public void testCase() throws InterruptedException, IOException {
        ...
        BufferedReader readerlogin = new BufferedReader(new FileReader("D:\\login.csv"));
        WebDriver driver = new ChromeDriver();
        driver.get("http://148.251.88.9:8080/mfc_hmao/admin/"); //"http://mfc.admhmao.ru/mfctablet/quality.htm"
        String linelogin;
        while ((linelogin = readerlogin.readLine()) != null) {
            String strlogin[] = linelogin.split(";");
            if (strlogin.length > 1) {
                driver.findElement(By.name("login")).clear();
                driver.findElement(By.name("login")).sendKeys(strlogin[0]);//"login", "fileNumber"
                driver.findElement(By.name("password")).clear();
                driver.findElement(By.name("password")).sendKeys(strlogin[1]);//"password", "authCode"
